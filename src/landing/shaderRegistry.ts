...
`,
  ...
  // --- Phantom IFS boxes (raymarched volumetric) ---
  {
    id: 'phantom-ifs',
    name: 'Phantom IFS',
    ...
    gamma: 2.9,
    display: {
      fractalResolution: 1024 /2,
      triAlpha: { min: 0.0, range: 0.45 },
      triBlending: 'additive',
    },
    source: /* glsl */ `
mat2 rot(float a) {
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 cPos = vec3(0.0,0.0, -3.0 * iTime*0.1);
    vec3 cDir = normalize(vec3(0.0, 0.0, -1.0));
    vec3 cUp  = vec3(sin(iTime/5.), 1.0, 0.0);
    ...
    for (int i = 0; i < 99; i++) {
        ...
        if (mod(length(pos)+1.21*iTime, 120.0) < 2.9) {
            a *= 2.0;
            acc2 += a;
        }
        acc += a;
